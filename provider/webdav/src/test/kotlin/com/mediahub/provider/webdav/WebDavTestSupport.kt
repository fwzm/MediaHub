...
internal class WebDavTestStack(val baseUrl: String, val username: String? = "alice") {
    ...
    val credentialCoordinator = WebDavCredentialCoordinator()
    val credentialStore = WebDavCredentialStore(vault, credentialCoordinator)
    ...
    suspend fun storePassword(password: String = PASSWORD) {
        // 必须走 credentialStore（而非直写 vault）：世代与身份绑定是生产路径的一部分
        credentialStore.savePassword(server, password)
    }
