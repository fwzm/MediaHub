...
class JellyfinFactoryRestoreIsolationTest {
    ...
    private val factory = JellyfinProviderFactory(HttpClientFactory(logger), tokens,
        ClientIdentity("MediaHub", "test", "test-device", "test"), jellyfinSessionStorage = sessions, logger = logger)
    ...
    private fun server(endpoint: MockWebServer) = MediaServer(
        id = "source", name = "test", type = ServerType.JELLYFIN,
    ...
    private suspend fun newIdentityCredentials() {
        ...
        JellyfinSessionStore(sessions).save(JellyfinSession("source", "new-remote", "new-user", "new-account"))
    }
    ...
    @Test fun `old factory handle cannot send new identity token to its captured address`() = runBlocking {
        ...
        // Jellyfin library 路径的公开契约把它映射为 ProviderException.Network 且保留 cause。
    ...
    private class MemorySessions : JellyfinSessionStore.Storage {
        private val values = mutableMapOf<String, String>()
